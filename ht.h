...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if(i != npos && table_[i] != NULL) {
        table_[i]->item.second = p.second;
    } else {
        double load = static_cast<double>(items_ + filled_) / static_cast<double>(CAPACITIES[mIndex_]);
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    mIndex_++;
    HASH_INDEX_T sizedUp = CAPACITIES[mIndex_];
    
    std::vector<HashItem*> prevTab = table_;
    table_.assign(sizedUp, NULL);
    filled_ = 0;
    items_ = 0;
    
    for(size_t i = 0; i < prevTab.size(); ++i){
        HashItem* item = prevTab[i];

        if(item != NULL){

            if(!(item->deleted)){
                HASH_INDEX_T hash = hash_(item->item.first) % sizedUp;
                prober_.init(hash, sizedUp, item->item.first);
                HASH_INDEX_T current = prober_.next();
                totalProbes_++;

                while(current != npos && table_[current] != NULL){
                    current = prober_.next();
                    totalProbes_++;
                }

                if(current == npos){
                    throw std::logic_error("Failed to Resize");
                }
                table_[current] = item;
                items_++;
            } else {
